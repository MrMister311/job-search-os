...
file is that channel's scoreboard.

## Rules

1. Every application's **warm-path pass** (CLAUDE.md Step 4) logs its people here, found or
...
3. Warm-path mechanic (no product needed): rank people by overlap (shared employer, school,
   city, community, title adjacency), identify the likely hiring manager from the JD and
   LinkedIn, draft the outreach note in the same session as the tailoring.
...
## Prospect pools to mine (fill at intake: alumni networks, past employers, communities)
